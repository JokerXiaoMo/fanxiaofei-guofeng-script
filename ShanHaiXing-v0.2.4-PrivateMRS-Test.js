...
// 说明：这是 v0.2.4 私有网络 MRS 规则优化测试版；当前正式入口保持不变。
// 改进：在不替换 Geo 数据库的前提下，补齐私有域名与私有 IP 的优先直连保护。

var VERSION = '0.2.4-private-mrs-test'
...
function mrsProvider(path, localPath, behavior) {
  return {
    type: 'http',
    behavior: behavior,
    format: 'mrs',
    // 数据来源：MetaCubeX/meta-rules-dat（GPL-3.0）；仅按 Mihomo MRS 格式外部引用，不复制其生成实现。
...
function installRules(config) {
  config['rule-providers'] = {
    'shanhai-private-domain': mrsProvider('geo/geosite/private.mrs', 'private-domain.mrs', 'domain'),
    'shanhai-private-ip': mrsProvider('geo/geoip/private.mrs', 'private-ip.mrs', 'ipcidr'),
  ...
  var rules = [
    ...
    // 私有域名/IP 先直连，避免 loopback、链路本地及保留目标误入广告或代理业务规则。
    'RULE-SET,shanhai-private-domain,DIRECT',
    'IP-CIDR,10.0.0.0/8,DIRECT,no-resolve',
    'IP-CIDR,172.16.0.0/12,DIRECT,no-resolve',
    'IP-CIDR,192.168.0.0/16,DIRECT,no-resolve',
    'RULE-SET,shanhai-private-ip,DIRECT,no-resolve',
