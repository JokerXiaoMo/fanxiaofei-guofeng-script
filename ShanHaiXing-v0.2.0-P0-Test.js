...
// 说明：这是 v0.2.0 的 P0 隔离测试版，仅验证内置非远端节点不会进入测速候选。
// 当前正式入口 ShanHaiXing-Adaptive-DualStackDNS.js 不受本文件影响。

var VERSION = '0.2.0-p0-test-builtin-node-filter'
...
// 这些类型是 Mihomo 的内置出口或策略组类型，不是机场远端节点。
// 它们若进入 url-test，会产生 DIRECT 测速、必然失败的 REJECT 或策略组嵌套污染。
var BUILTIN_NON_REMOTE_TYPES = [
  'direct', 'reject', 'reject-drop', 'pass', 'rematch',
  'select', 'selector', 'url-test', 'fallback', 'load-balance', 'relay', 'compatible'
]

function isBuiltinNonRemoteProxy(proxy) {
  if (!proxy || typeof proxy.type !== 'string') return false
  return BUILTIN_NON_REMOTE_TYPES.indexOf(proxy.type.toLowerCase()) !== -1
}
...
function collectNodes(proxies) {
  ...
  for (var i = 0; i < proxies.length; i += 1) {
    ...
    if (isBuiltinNonRemoteProxy(proxy)) {
      log('跳过内置非远端节点：' + nodeName + ' (' + proxy.type + ')')
      continue
    }
