...
| `☁️ 万象节点` | 低倍率偏好的全节点延迟自动池 | 先直接测试低倍率真实节点，再测试全部非低倍率有效节点 | `url-test` 只比较真实节点延迟；低倍率节点在延迟接近时优先列出，若高延迟或失效会自动切换到更低延迟节点 |
...
>
> **自动选择优先：** 脚本将 `profile.store-selected` 设为 `false`，防止 Mihomo/BettBox 将旧的手动高延迟选择恢复到 `url-test` 自动组。代价是 `🌺 代理选择` 等手动组的选择不会跨刷新保存；这是为懒人用户保证自动测速有效所作的明确取舍。
...
| 首页策略组 | `🌺 代理选择` 位于第一项 |
| 万象延迟感知 | 有低倍率节点时，`☁️ 万象节点` 会先直接测试低倍率真实节点并持续比较全部真实候选；高延迟或失效时自动切换 |
| 自动选择重置 | 刷新覆写后，旧的手动选中状态不应继续固定 `url-test` 自动组 |
