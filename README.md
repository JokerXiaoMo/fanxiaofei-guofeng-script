...
| `☁️ 万象节点` | 低倍率优先的全节点自动回退池 | 先引用 `🟢 低倍率节点`，随后保留订阅内全部有效节点 | `fallback` 优先使用低倍率组中延迟最优且健康的节点；低倍率组不可用或不存在时自动回退至其他有效节点 |
...
| 首页策略组 | `🌺 代理选择` 位于第一项 |
| 万象成本优先 | 有低倍率节点时，`☁️ 万象节点` 的第一候选为 `🟢 低倍率节点`；不存在或不健康时自动回退 |
