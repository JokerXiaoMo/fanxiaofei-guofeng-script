// FlClash 覆写脚本：山海行 · 兼容测试版
// 适用：FlClash v0.8.85+（标准 Mihomo 内核）
// 设计：仅引用实际生成的地区组，避免订阅缺少特定地区节点时出现策略组不存在错误。
// 注意：这是独立测试文件，不替代 ShanHaiXing.js 原版。

var VERSION = '1.0.0-compat'
...
function log(message) {
  if (typeof console !== 'undefined' && console.log) console.log('[山海行兼容测试 ' + VERSION + '] ' + message)
}
