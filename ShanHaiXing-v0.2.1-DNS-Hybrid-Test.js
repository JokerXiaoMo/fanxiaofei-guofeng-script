...
// 说明：这是 v0.2.1 DNS 分层隔离测试版；当前正式入口保持不变。
// 目标：以用户指定的纯 IP DNS 负责引导与代理节点解析，普通域名查询保持 DoH 加密。

var VERSION = '0.2.1-dns-hybrid-test'
var TEST_URL = 'https://www.gstatic.com/generate_204'
var TEST_INTERVAL = 600
var TEST_TOLERANCE = 50

// 用户指定的纯 IP DNS 仅用于解析 DNS 服务端与代理节点域名：
// 223.5.5.5 为 IPv4 首项，1.1.1.1 为 IPv4 复选，2400:3200::1 为 IPv6 上游。
// Mihomo 对同一数组上游可并发查询，数组顺序不等于严格主备；不可据此承诺固定先后。
var DNS_DEFAULT_NAMESERVER = ['223.5.5.5', '1.1.1.1', '2400:3200::1']
var DNS_PROXY_SERVER_NAMESERVER = ['223.5.5.5', '1.1.1.1', '2400:3200::1']

// 普通域名查询走 DoH，避免把纯 IP UDP/TCP DNS 误称为防泄露方案。
var DNS_NAMESERVER = ['https://dns.alidns.com/dns-query', 'https://cloudflare-dns.com/dns-query']

// 根级 hosts 在 Mihomo DNS 处理前提供端点地址，避免首次解析 DoH 端点时发出明文引导查询。
// 保持域名形式可继续执行 TLS 证书与 SNI 校验，不使用跳过证书校验的 IP DoH 写法。
var DNS_BOOTSTRAP_HOSTS = {
...
function installDnsBootstrapHosts(config) {
  if (!config.hosts || typeof config.hosts !== 'object' || Array.isArray(config.hosts)) config.hosts = {}
  for (var host in DNS_BOOTSTRAP_HOSTS) {
    if (Object.prototype.hasOwnProperty.call(DNS_BOOTSTRAP_HOSTS, host) && !config.hosts[host]) {
      config.hosts[host] = DNS_BOOTSTRAP_HOSTS[host].slice()
    }
  }
}
...
function applySafeDefaults(config) {
  ...
  // 根级 hosts 先提供 DoH 端点地址；已有同名自定义映射保持不变。
  installDnsBootstrapHosts(config)
